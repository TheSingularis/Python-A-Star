    ...
    def draw_grid(self, win, switch):
        for col in range(len(self.grid)):
            for row in range(len(self.grid[0])):
                self.grid[col][row].draw(win, switch.get(self.grid[col][row].state))
        ...
        # sleep here to make it slower for viewing purposes

    def start_or_end_found(self, block_type):
        for col in self.grid:
            for value in col:
                if value.state == block_type:
                    return True, value
